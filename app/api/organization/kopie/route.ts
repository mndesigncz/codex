...
// a jen jako podnik, kde je volající členem s oprávněním. Kolo 67: místo
// „vedení v obou" organizace.kopirovat v obou podnicích, ve ZDROJI právo
// danou věc vidět (*.zobrazit) a v CÍLI ji zakládat (návody/postupy
// .vytvorit, menu .upravit + .ceny, protože kopie menu nese i ceny). Role
// v jedné pobočce tak dál neotevírá receptury, postupy a ceny ostatních.
...
/** Co je potřeba ve zdroji a v cíli podle toho, co se kopíruje. */
const PRAVA: Record<EntitaKopie, { zdroj: string[]; cil: string[] }> = {
  navody: { zdroj: ['navody.zobrazit'], cil: ['navody.vytvorit'] },
  postupy: { zdroj: ['postupy.zobrazit'], cil: ['postupy.vytvorit'] },
  menu: { zdroj: ['menu.zobrazit'], cil: ['menu.upravit', 'menu.ceny'] },
};
...
/** Společná brána GET i POST: oprávnění v cíli i ve zdroji, táž organizace, zdroj jiný než cíl a nepozastavený. */
async function zdroj(entitaRaw: unknown, zRaw: unknown): Promise<Zdroj | Chyba | NextResponse> {
  const c = await pozaduj('organizace.kopirovat');
  if (jeOdpoved(c)) return c;
  if (!jeEntitaKopie(entitaRaw)) return { error: 'Kopírovat jdou návody, postupy nebo menu.', status: 400 };
  const potreba = PRAVA[entitaRaw];
  if (!potreba.cil.every(k => c.role.opravneni.has(k))) {
    return { error: 'V tomhle podniku nemáš oprávnění tohle zakládat.', status: 403 };
  }
  const z = Number(zRaw);
  ...
  // Ve zdroji rozhoduje členství v databázi a role v TOM podniku — ne role
  // z tokenu ani z aktivního podniku.
  const clenstvi = await clenstviUzivatele(c.meId);
  const rz = clenstvi.some(m => m.teamId === z && m.organizationId === org.id) ? await roleClena(c.meId, z) : null;
  if (!rz || !['organizace.kopirovat', ...potreba.zdroj].every(k => rz.opravneni.has(k))) {
    return { error: 'Kopírovat jde jen z podniku organizace, kde na to máš oprávnění.', status: 403 };
  }
...
const jeChyba = (v: Zdroj | Chyba | NextResponse): v is Chyba => !(v instanceof NextResponse) && 'error' in v;

export async function GET(request: Request) {
  const { searchParams } = new URL(request.url);
  const v = await zdroj(searchParams.get('entita'), searchParams.get('z'));
  if (jeOdpoved(v)) return v;
