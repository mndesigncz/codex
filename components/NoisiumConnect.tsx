    ...
    <div className="glass-card p-6 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="font-bold tracking-tight text-[#16181A] flex items-center gap-2">
            <Icon name="kanban" size={18} /> Propojení s Noisium
          </h3>
          <p className="text-black/45 text-sm mt-1">Publikuj úkoly z Plánování přímo do Noisium (Plan app).</p>
        </div>
        {connected && <span className="rounded-full px-3 py-1 text-xs font-medium bg-[#C8F542]/15 text-[#5B7A08] whitespace-nowrap">Připojeno</span>}
      </div>
      ...
      {connected ? (
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <p className="text-sm text-black/60">Projekt v Noisium je propojený{projectId ? ` (ID ${projectId})` : ''}. V Plánování teď máš u karet tlačítko „Publikovat".</p>
          <button onClick={disconnect} disabled={busy} className="rounded-full glass border border-black/10 text-red-600 px-4 py-2 text-sm font-medium hover:bg-red-500/[0.06] disabled:opacity-50">Odpojit</button>
        </div>
      ) : (
        <form onSubmit={connect} className="space-y-3">
          ...
          <button type="submit" disabled={busy || !token.trim()} className="rounded-full bg-[#16181A] text-white font-semibold px-5 py-2.5 text-sm hover:bg-black disabled:opacity-50">
