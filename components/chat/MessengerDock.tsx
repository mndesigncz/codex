      ...
      <div className="flex items-end gap-3">
        ...
        <div className="flex flex-col items-end gap-3">
          ...
          >
            ...
            {totalUnread > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1.5 rounded-full bg-red-500 text-white text-[11px] font-semibold flex items-center justify-center border-2 border-[#0A0A0C]">
                {totalUnread}
              </span>
            )}
          </button>
  ...
  return (
    <div className="w-80 max-h-[70vh] rounded-3xl glass-strong shadow-[0_12px_40px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col animate-[chatDockIn_.18s_cubic-bezier(0.16,1,0.3,1)]">
      <div className="px-4 py-3 flex items-center justify-between border-b border-white/[0.06]">
        <span className="font-semibold text-white">Zprávy</span>
        <button
          onClick={onClose}
      ...
      <div className="overflow-y-auto scrollbar-thin divide-y divide-white/[0.06]">
...
function ChatWindow({
  ...
  return (
    ...
    >
      <header className="px-3 py-2.5 flex items-center gap-2 border-b border-white/[0.06]">
        <ConvAvatar conv={conv} size={32} />
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-white text-sm truncate">{conv.name}</div>
          {conv.type === 'team' && (
            <div className="text-[10px] text-white/40 leading-none">Týmový kanál</div>
          )}
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin px-3 py-3 space-y-1.5">
        {messages.length === 0 && (
          <div className="text-center text-white/40 text-xs py-6">Zatím žádné zprávy.</div>
        )}
        {messages.map((m, i) => (
          <MessageBubble
            key={m.id}
      ...
      <form
        onSubmit={handleSend}
        className="px-2.5 py-2.5 border-t border-white/[0.06] flex items-center gap-1.5"
      >
        ...
        <button
          ...
          disabled={uploading}
          className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-white/60 hover:text-white hover:bg-white/[0.06] transition-colors disabled:opacity-40"
        ...
        <input
          ...
          placeholder="Zpráva…"
          className="flex-1 min-w-0 rounded-2xl bg-white/[0.06] border border-white/10 px-3 py-2 text-sm text-white placeholder-white/30 focus:border-[#C8F542]/50 focus:ring-2 focus:ring-[#C8F542]/20 focus:outline-none"
        />
        <button
          type="submit"
