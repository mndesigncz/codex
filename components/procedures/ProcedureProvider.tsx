...
export function ProcedureProvider({ children }: { children: React.ReactNode }) {
  ...
  const persist = useCallback((runId: number, checkedItems: number[]) => {
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => {
      fetch('/api/procedures/runs', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ runId, checkedItems }),
      }).catch(() => { /* best-effort */ });
    }, 350);
  }, []);
  ...
  const complete = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/procedures/runs', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ runId: snapshot.id, checkedItems: snapshot.checkedItems, complete: true }),
      });
      const data = await res.json();
      ...
      setJustCompleted({ name: snapshot.name, duration, total: snapshot.totalItems });
      setActive(null);
    } catch {
      const duration = Math.max(0, Math.round((Date.now() - new Date(snapshot.startedAt).getTime()) / 1000));
      setJustCompleted({ name: snapshot.name, duration, total: snapshot.totalItems });
      setActive(null);
    }
  }, [active]);
