...
function StatCard({ icon, label, value, sub, progress = 100, alert = false }: { icon: string; label: string; value: string | number; sub?: string; progress?: number; alert?: boolean }) {
  return (
    <div className="glass-card p-6 hover:bg-black/[0.05] transition-all duration-300">
      ...
      <p className="text-3xl md:text-4xl font-bold tracking-tight mt-3 text-[#16181A]">{value}</p>
      {sub && <p className="text-xs text-black/45 mt-1">{sub}</p>}
        ...
        <>
          ...
          <div className="glass-card p-6 hover:bg-black/[0.05] transition-all duration-300">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold tracking-tight text-[#16181A]">Dnešní směny</h3>
              <button onClick={() => onNavigate('shifts')} className="text-sm text-[#5B7A08] hover:brightness-110 transition-all">Všechny směny →</button>
            </div>
            {todayShifts.length === 0 ? (
              <p className="text-black/45 text-sm">Dnes nejsou žádné naplánované směny.</p>
            ) : (
              <div className="space-y-3">
                {todayShifts.map(s => (
                  <div key={s.id} className="flex items-center gap-3 p-3 bg-black/[0.04] rounded-2xl hover:bg-black/[0.05] transition-all duration-300">
                    <span className="text-2xl">{s.type === 'morning' ? '🌅' : '🌆'}</span>
                    <div>
                      <p className="font-semibold text-[#16181A] text-sm">{getUserName(s.employeeId)}</p>
                      <p className="text-xs text-black/45">{s.startTime} – {s.endTime} · {s.type === 'morning' ? 'Ranní' : 'Odpolední'} směna</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="glass-card p-6 hover:bg-black/[0.05] transition-all duration-300">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold tracking-tight text-[#16181A]">Aktivní úkoly</h3>
                <button onClick={() => onNavigate('team')} className="text-sm text-[#5B7A08] hover:brightness-110 transition-all">Zobrazit →</button>
              </div>
              <div className="space-y-2">
                {pendingTasks.slice(0, 4).map(t => (
                  <div key={t.id} className="flex items-center gap-3 p-3 bg-black/[0.04] rounded-2xl hover:bg-black/[0.05] transition-all duration-300">
                    <span className="text-lg">{t.priority === 'high' ? '🔴' : t.priority === 'medium' ? '🟡' : '🟢'}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[#16181A] truncate">{t.title}</p>
                      {t.dueDate && <p className="text-xs text-black/45">Do: {t.dueDate}</p>}
                    </div>
                    <span className={`rounded-full px-3 py-1 text-xs font-medium ${t.status === 'in_progress' ? 'bg-blue-500/15 text-blue-600' : 'bg-black/[0.06] text-black/60'}`}>
                      {t.status === 'in_progress' ? 'Probíhá' : 'Čeká'}
                    </span>
                  </div>
                ))}
                {pendingTasks.length === 0 && <p className="text-black/45 text-sm">Žádné aktivní úkoly 🎉</p>}
              </div>
            </div>
            ...
            <div className="glass-card p-6 hover:bg-black/[0.05] transition-all duration-300">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => onNavigate('inventory')} className="text-sm text-[#5B7A08] hover:brightness-110 transition-all">Sklad →</button>
              </div>
              <div className="space-y-2">
                {lowStock.slice(0, 5).map(i => (
                  <div key={i.id} className="flex items-center gap-3 p-3 bg-red-500/10 rounded-2xl hover:bg-red-500/15 transition-all duration-300">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium text-[#16181A]">{i.name}</p>
                      <p className="text-xs text-red-600">{i.quantity} {i.unit} / min. {i.minQuantity} {i.unit}</p>
                    </div>
                  </div>
                ))}
                {lowStock.length === 0 && <p className="text-black/45 text-sm">Zásoby jsou v pořádku</p>}
              </div>
            </div>
          </div>
        </>
