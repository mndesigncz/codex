...
const inputClass = 'w-full rounded-2xl bg-white/[0.06] border border-white/10 px-4 py-3 text-white placeholder-white/30 focus:border-[#C8F542]/50 focus:ring-2 focus:ring-[#C8F542]/20 focus:outline-none transition-all text-sm';
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">Sklad & zásoby</h1>
          <p className="text-white/40 text-sm">Přidávejte položky a nastavte hlídací limity</p>
        </div>
      ...
      {(critical.length > 0 || low.length > 0) && (
        <div className="glass-card border-orange-500/20 bg-orange-500/[0.06] p-5">
          <p className="font-semibold text-sm text-orange-300">
            {critical.length > 0 && <span className="text-red-400">{critical.length} kriticky málo</span>}
            {critical.length > 0 && low.length > 0 && ' · '}
            {low.length > 0 && <span className="text-orange-400">{low.length} dochází</span>}
          </p>
          <p className="text-white/50 text-sm mt-1">{[...critical, ...low].map(i => i.name).join(', ')}</p>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-1 overflow-x-auto glass rounded-full p-1">
          {['Vše', ...CATEGORIES].map(c => (
            <button key={c} onClick={() => setCat(c)} className={`px-4 py-2 rounded-full text-xs font-medium whitespace-nowrap transition-all ${cat === c ? 'bg-[#C8F542] text-black' : 'text-white/50 hover:text-white'}`}>{c}</button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass-card p-8 text-center text-white/40">Žádné položky. Přidejte první.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map(i => {
            ...
            const chip = st === 'critical' ? 'bg-red-500/15 text-red-400' : st === 'low' ? 'bg-orange-500/15 text-orange-400' : 'bg-[#C8F542]/15 text-[#C8F542]';
            return (
              <div key={i.id} className="glass-card p-5">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="font-semibold text-white">{i.name}</p>
                    <p className="text-xs text-white/40">{i.category}{i.supplier ? ` · ${i.supplier}` : ''}</p>
                  </div>
                ...
                <div className="mt-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button onClick={() => step(i, -1)} className="rounded-full glass w-8 h-8 flex items-center justify-center text-white/70 hover:text-white">−</button>
                    <span className="text-lg font-bold text-white w-16 text-center">{i.quantity} <span className="text-xs text-white/40">{i.unit}</span></span>
                    <button onClick={() => step(i, 1)} className="rounded-full glass w-8 h-8 flex items-center justify-center text-white/70 hover:text-white">+</button>
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(i)} className="rounded-full glass w-9 h-9 flex items-center justify-center text-white/60 hover:text-white text-sm">✎</button>
                    <button onClick={() => remove(i)} className="rounded-full glass w-9 h-9 flex items-center justify-center text-red-400/70 hover:text-red-400 text-sm">✕</button>
                  </div>
                </div>
                <p className="text-[11px] text-white/25 mt-2">Limit: {i.minQuantity} · kriticky: {i.criticalQuantity} {i.unit}{i.updatedByName ? ` · upravil ${i.updatedByName}` : ''}</p>
              </div>
            );
          })}
        </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-50 flex items-end md:items-center justify-center md:p-4" onClick={() => setShowForm(false)}>
          <form onClick={e => e.stopPropagation()} onSubmit={save} className="glass-strong rounded-3xl rounded-b-none md:rounded-3xl w-full max-w-lg p-6 space-y-4 max-h-[85vh] overflow-y-auto scrollbar-thin">
            <h3 className="text-lg font-bold tracking-tight text-white">{editing ? 'Upravit položku' : 'Nová položka'}</h3>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <label className="block text-xs uppercase tracking-wider text-white/40 mb-1.5">Název</label>
                <input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className={inputClass} />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wider text-white/40 mb-1.5">Kategorie</label>
                <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} className={inputClass}>
                  {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wider text-white/40 mb-1.5">Jednotka</label>
                <input value={form.unit} onChange={e => setForm(f => ({ ...f, unit: e.target.value }))} className={inputClass} />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wider text-white/40 mb-1.5">Aktuální množství</label>
                <input type="number" value={form.quantity} onChange={e => setForm(f => ({ ...f, quantity: e.target.value }))} className={inputClass} />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wider text-white/40 mb-1.5">Max. množství</label>
                <input type="number" value={form.maxQuantity} onChange={e => setForm(f => ({ ...f, maxQuantity: e.target.value }))} className={inputClass} />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wider text-orange-400/70 mb-1.5">Upozornit při</label>
                <input type="number" value={form.minQuantity} onChange={e => setForm(f => ({ ...f, minQuantity: e.target.value }))} className={inputClass} />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wider text-red-400/70 mb-1.5">Kriticky málo při</label>
              ...
              <div className="col-span-2">
                <label className="block text-xs uppercase tracking-wider text-white/40 mb-1.5">Dodavatel (volitelné)</label>
                <input value={form.supplier} onChange={e => setForm(f => ({ ...f, supplier: e.target.value }))} className={inputClass} />
              </div>
            </div>
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={() => setShowForm(false)} className="flex-1 rounded-full glass border border-white/15 text-white py-3 text-sm font-medium hover:bg-white/10">Zrušit</button>
